---
layout: homepage
---

<h1 id="about-me"></h1>

<h2 style="margin: 60px 0px 10px;">Short Bio</h2>

<!-- I am currently a Physics Ph.D. candidate at [Spin-Ion Technologies](https://www.spin-ion.com/) and [Université Paris-Saclay](https://www.universite-paris-saclay.fr/), working under the mentorship of Dr. [Ravelosona dafiné](https://www.spin-ion.com/about-us/), the director of research [CNRS](https://www.cnrs.fr/fr) and CTO of [Spin-Ion Technologies](https://www.spin-ion.com/). -->

I am an undergraduate student at [Fudan University](https://www.fudan.edu.cn/), advised by [Prof. Hao Zhang](https://optics.fudan.edu.cn/2a/11/c39771a469521/page.htm).
Currently, I work with the Computational Design and Fabrication Group [(CDFG)](https://cdfg.csail.mit.edu) at [MIT CSAIL](https://www.csail.mit.edu/), under the supervision of Prof. [Wojciech Matusik.](https://cdfg.csail.mit.edu/wojciech)
During the summer of 2025, I was a visiting student at Stanford University, where I deepened my knowledge in machine learning and computational modeling.

## Research Interest
- **AI for Autonomous Scientific Discovery and Mechanism Inference:**- Integrating physics and materials science with advanced AI to enable autonomous scientific discovery. My work moves beyond correlation (e.g., CTGNN, VQCrystal) towards the mechanistic understanding of complex systems. I focus on building closed-loop hybrid architectures that learn, refine, and evolve knowledge, mimicking the human scientific process.
- **Constrained Large Language Models (LLMs) and Neuro-Symbolic Systems:**- Developing robust methods to harness LLMs for high-level scientific reasoning and decision-making. My contributions include engineering knowledge-graph-enhanced RAG systems and LoRA fine-tuning for improved scientific query accuracy, while addressing the core limitations of unconstrained LLMs. I specialize in neuro-symbolic systems and the design of dual-layer language systems (AxiomDSL and AxiomIR) to enforce logical rigor and structural consistency in machine-generated hypotheses.
- **Energy Systems and Advanced Materials Engineering:**- Applying AI and rigorous system design to accelerate the discovery and optimization of functional materials essential for energy and chemical processes. This includes experience in materials modeling and generation, as well as multi-mode system engineering demonstrated by the successful development of the CPV-TE hybrid cell system (increasing efficiency from 25% to 38%). I focus on integrating hard physical constraints and simulation-grounded verification to ensure that new material hypotheses are truly testable and physically valid.
  
## Honors and Awards
- **Huatai Securities Science and Technology Scholarship (First Prize)** — Awarded to **top1%** students with exceptional achievements in scientific research and innovation.  
- **Puyuan Talent Scholarship (Second Prize)** — Recognized for outstanding academic performance and interdisciplinary research potential. **6 Students** of nearly 600 students in my instituition. 
- **Silver Prize**, *National College Students “Internet Plus” Innovation and Entrepreneurship Competition*, Shanghai Division, 2025  
- **Top10 Students Award**, *School of Information Science and Engineering*, Fudan University — For academic excellence and community contribution.  
- **Outstanding Student Cadre**, Fudan University, 2023 — Recognition of leadership and service in student organizations.  
- **H Prize**, *Mathematical Contest in Modeling (MCM)*, 2024


{% include_relative _includes/publications.md %}

{% include_relative _includes/projects.md %}

{% include_relative _includes/contact.md %}
